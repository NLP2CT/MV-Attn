...
from thumt.layers.multi_tasks_nn import for_back_ward, long_term, short_term,temperature,local_ness,gate_sum,sub_gate_sum,sub_gate_sum_m
...
def all_mask(q,k,heads=1,number_heads=8,scope=None):
    #1
    short_mask =short_term(q,1,number_heads)
    #2
    long_mask = long_term(q,1,number_heads)
    # 3-4
    for_back_mask = for_back_ward(q,heads,number_heads)
    #
    local_mask = local_ness(q,k,4,heads,number_heads)
...
def multiplicative_attention(queries, keys, values, bias, mask, keep_prob=None,multi_drop=None,heads=1,num_heads=8,
                             name=None,ori=None):
    ...
    with tf.name_scope(name, default_name="multiplicative_attention",
        ...
        if heads==4:
            final_outs = gate_sum(outputs,multi_outs,ori)
        else:
            if ori is None:
                final_outs = sub_gate_sum_m(outputs,multi_outs,tf.concat([outputs,multi_outs],axis=-1))
            else:
                final_outs = sub_gate_sum(outputs,multi_outs,ori,heads=8,num_heads=num_heads)
...
def multihead_attention(queries, memories, bias, num_heads, key_size,
    ...
    with tf.variable_scope(scope, default_name="multihead_attention",
        ...
        if mode == 'temper' or mode== 'phrase':
            ...
            multi_mask = all_mask(q,k,heads,num_heads)
        else:
            multi_mask = mask_fun[mode](q,heads,num_heads)
        ...
        if cate:
            ...
            results = multiplicative_attention(q, k, v, bias, multi_mask, keep_prob,multi_drop,heads,ori=queries,num_heads=num_heads)
