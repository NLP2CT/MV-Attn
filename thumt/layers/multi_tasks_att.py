...
from thumt.layers.multi_tasks_nn import for_back_ward, long_term, short_term,temperature,local_ness
...
def all_mask(q,k,heads,number_heads,scope=None):
    #1
...
def multiplicative_attention(queries, keys, values, bias, mask, keep_prob=None,
                             name=None):
...
def multihead_attention(queries, memories, bias, num_heads, key_size,
                        value_size, output_size, keep_prob=None, mode='all',heads=1, output=True,
    ...
    with tf.variable_scope(scope, default_name="multihead_attention",
        ...
        # attention
        results = multiplicative_attention(q, k, v, bias, multi_mask, keep_prob)
