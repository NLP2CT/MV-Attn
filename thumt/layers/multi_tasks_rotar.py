...
from thumt.layers.nn import linear,layer_norm
from thumt.layers.multi_tasks_nn import linear, for_back_ward, long_term, short_term,temperature,local_ness,gate_sum,sub_gate_sum,sub_gate_sum_m

mask_fun = {
    'for_back': eval('for_back_ward'),
    'longterm': eval('long_term'),
    'shorterm': eval('short_term'),
    'temper': eval('temperature'),
    'phrase':eval('local_ness')
}

def all_mask(q,k,heads=1,scope=None):
    #1
    short_mask =short_term(q,1)
    #2
    long_mask = long_term(q,1)
    # 3-4
    for_back_mask = for_back_ward(q,1)
    #
    local_mask = local_ness(q,k,4,heads)
    mask = tf.concat([short_mask,long_mask,for_back_mask,local_mask],axis=1)
    return mask
...
def dynamic_routing(output_heads):  # [batch, length, heads * channels]
    with tf.variable_scope("dynamic_routing"):
        ...
        combined_output = _ffn_layer_2(
            _layer_process_2(
                tf.reshape(output_heads, [tf.shape(output_heads)[0], tf.shape(output_heads)[1], channels * heads])
                , "layer_norm"),
            heads * channels,  # 512
        ...
        combined_output = tf.reshape(combined_output,
                                     [tf.shape(output_heads)[0], tf.shape(output_heads)[1], num_capsules, heads, 1])
        ...
        outputs = _layer_process_2(v, 'none')  # if necessary?
...
def make_mask(inputs, heads, mode='all', scope=None):
    num_head = inputs.get_shape().as_list()[1]
    lenght = tf.shape(inputs)[2]

    init_mask = tf.ones([tf.shape(inputs)[0],heads - num_head, lenght, lenght])

    mask = tf.concat([inputs, init_mask], axis=1)
    return mask
...
def multiplicative_attention(queries, keys, values, bias, mask, keep_prob=None,multi_drop=None,heads=1,
                             name=None,ori=None):
    ...
    with tf.name_scope(name, default_name="multiplicative_attention",
                       values=[queries, keys, values, bias,mask]):
        # shape: [batch, heads, length_q, length_kv]

        logits = tf.matmul(queries, keys, transpose_b=True)
        # multi_view = tf.identity(logits)
        if bias is not None:
            logits += bias
        multi_view = logits+mask
        weights = tf.nn.softmax(logits,name="attention_weights")
        multi_weight = tf.nn.softmax(multi_view,name='multi_view_att_w')
        if keep_prob is not None and keep_prob < 1.0:
            weights = tf.nn.dropout(weights, keep_prob)
            multi_weight = tf.nn.dropout(multi_weight,multi_drop)
        outputs = tf.matmul(weights, values)
        multi_outs = tf.matmul(multi_weight,values)
        final_outs = sub_gate_sum(outputs,multi_outs,ori)

        return {"weights": weights, "outputs": final_outs}


def multihead_attention(queries, memories, bias, num_heads, key_size,
                        value_size, output_size, keep_prob=None, multi_drop=None,mode='all',heads=1,cate=True, output=True,
    ...
    with tf.variable_scope(scope, default_name="multihead_attention",
        ...
        if mode == 'temper' or mode== 'phrase':
            multi_mask = mask_fun[mode](q, k, 4, heads)
        elif mode =='all':
            multi_mask = all_mask(q,k,heads)
        else:
            multi_mask = mask_fun[mode](q,heads)

        multi_mask = make_mask(multi_mask, num_heads, mode)
        # scale query
        ...
        # attention
        if cate:
            results = multiplicative_attention(q, k, v, bias, multi_mask, keep_prob,multi_drop,heads,ori=None)

        else:
            results = multiplicative_attention(q, k, v, bias, multi_mask, keep_prob,multi_drop,heads,ori=queries)
