...
def em_routing(output_heads, params):  # [batch, length, heads * channels]
    with tf.variable_scope("em_routing"):
        ...
        for i in range(routing_iter):
            ...
            o_stdv = (tf.reduce_sum(r * tf.square(vote_in - o_mean), axis=2, keep_dims=True)) / (
                        r_sum + epsilon)  # [?, ?, 1, 512, 1]
            ...
            if i < routing_iter - 1:
                # E step
                o_p_unit0 = - tf.reduce_sum(tf.square(vote_in - o_mean) / (2 * o_stdv), axis=-1,
                                            keep_dims=True)  # [?, ?, 8, num, 1]
    ...
    with tf.variable_scope(scope, default_name="encoder", dtype=dtype,
        ...
        for layer in range(params.num_encoder_layers):
            with tf.variable_scope("layer_%d" % layer):
                with tf.variable_scope("multi_tasks_self_attention"):
                    y = layers.multi_tasks_loater.multihead_attention(
                        ...
                        multi_drop=1.0-params.attention_dropout*0.2,
                        mode=params.mode,
                        heads=params.heads,
                        cate=params.cate
                    )
